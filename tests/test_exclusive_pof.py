...
run_case('SUCCESS_MARKERS contract', t1)
...
run_case('PoC marker contract: exit 0 + VULNERABLE = exploited', t2)
...
def t3():
    ...
    shutil.rmtree(d, ignore_errors=True)
run_case('exit 1 + marker = not exploited', t3)
...
run_case('_eligible_for_autofix rules', t6)
...
run_case('gsc-autofix loop guard blocks re-processing', t7)
