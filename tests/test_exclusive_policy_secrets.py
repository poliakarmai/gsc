...
run_case('NL policy: accepts safe pattern', t1)
...
run_case('NL policy: rejects ReDoS pattern', t2)
...
run_case('REFINED_PATTERNS do NOT false-positively match SHA-256', t3)

def t4():
    sha = "9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08"
    for pat, st in REFINED_PATTERNS:
        assert re.search(pat, sha) is None, f"REFINED {st} gives FP on hash"
run_case('REFINED_PATTERNS: no FP on bare hash', t4)

def t5():
    fp = fingerprint_secret("AKIAIOSFODNN7EXAMPLE")
    assert len(fp) == 32
    assert "AKIA" not in fp
run_case('fingerprint_secret: irreversible hash', t5)
