...
test('compliance base + subrule fallback', t1)


def t2():
    assert compliance_for("GS999-unknown") == {}
test('compliance unknown rule → empty', t2)
...
test('enrich_finding injects compliance', t3)
...
test('GS029 entropy filter', t6)


def t7():
    det = GS029SecretsDetector()
    assert det.detect("tests/fixtures.py", 'aws = "AKIAIOSFODNN7EXAMPLE"') == []
test('GS029 excludes test paths', t7)
