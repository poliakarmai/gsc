...
def cmd_patterns(args):
    ...
    if action == 'review':
        ...
        subprocess.run([sys.executable, str(_GSC_ROOT / 'scripts' / 'gsc_marketplace.py'), 'export', getattr(args, 'file', '') or 'gsc_patterns.yaml'])
    elif action == 'import':
        subprocess.run([sys.executable, str(_GSC_ROOT / 'scripts' / 'gsc_marketplace.py'), 'import', getattr(args, 'file', '') or ''])
    else:
        subprocess.run([sys.executable, str(_GSC_ROOT / 'scripts' / 'gsc_marketplace.py')])
...
def cmd_registry(args):
    ...
    if args.reg_command == "list":
        ...
        for p in compiled:
            # Read name/severity without executing — from the header comment
            head = ""
        ...
        if not compiled:
            print("   (empty — compile with: gsc registry add <file.yml>)")

        # 2) Source YAML rules (own registry)
        ...
        for y in yamls:
            print(f"  • {y.name}")
        if not yamls:
            print("   (empty — create gsc-rules/<id>.yml)")

        # 3) How many YAML detectors are connected to the registry
...
def main():
    ...
    p_fed.set_defaults(func=lambda a: subprocess.run(
        [sys.executable, str(_GSC_ROOT / 'gsc_federated.py'),
    ...
    p_epss.set_defaults(func=lambda a: subprocess.run(
        [sys.executable, str(_GSC_ROOT / 'gsc_epss.py')]
    ...
    if args.command == "scan":
        ...
        arch_args = [sys.executable, str(_GSC_ROOT / "gsc_archaeology.py"), args.arch_cmd]
        ...
        pol_args = [sys.executable, str(_GSC_ROOT / "gsc_nlpolicy.py"), args.pol_cmd]
        ...
        sec_args = [sys.executable, str(_GSC_ROOT / "gsc_crossrepo_secrets.py"), args.sec_cmd]
        ...
        fc_args = [sys.executable, str(_GSC_ROOT / "gsc_forecast.py"), args.fc_cmd]
        ...
        subprocess.run([sys.executable, str(_GSC_ROOT / 'scripts' / 'gsc_config.py'), args.action or 'show', args.key or '', args.value or ''])

    elif args.command == 'metrics':
        subprocess.run([sys.executable, str(_GSC_ROOT / 'scripts' / 'gsc_metrics.py')])

    elif args.command == 'patterns':
        subprocess.run([sys.executable, str(_GSC_ROOT / 'scripts' / 'gsc_marketplace.py'), args.pat_action or 'list', getattr(args, 'file', '') or ''])

    elif args.command == 'issue':
        import importlib.util as _iu
        spec = _iu.spec_from_file_location('gsc_issue', str(_GSC_ROOT / 'scripts' / 'gsc_issue.py'))
        ...
        subprocess.run([sys.executable, str(_GSC_ROOT / "scripts" / "db_encrypt.py"), "encrypt"])
        ...
        if args.pof_cmd == 'generate':
            subprocess.run([sys.executable, str(_GSC_ROOT / "gsc_proofoffix.py"),
                            "generate", args.finding_key,
                            "--report", args.report,
                            "--project-root", args.project_root,
                            *(["--output", args.output] if hasattr(args, 'output') and args.output else [])])
        elif args.pof_cmd == 'batch':
            subprocess.run([sys.executable, str(_GSC_ROOT / "gsc_selfhealing.py"),
                            args.report,
        ...
        nuc_args = [sys.executable, str(_GSC_ROOT / "gsc_nuclei_export.py"), args.report]
        ...
        cmd = [sys.executable, str(_GSC_ROOT / "gsc_dast_scanner.py"), args.target]
        if hasattr(args, 'severity') and args.severity:
        ...
        cmd = [sys.executable, str(_GSC_ROOT / "gsc_nuclei_import.py"), "list"]
        if hasattr(args, 'severity') and args.severity:
            cmd += ["--severity", args.severity]
        ...
        if args.list:
            subprocess.run([sys.executable, str(_GSC_ROOT / "gsc_github_dorks.py"), "--list-dorks"])
        else:
            cmd = [sys.executable, str(_GSC_ROOT / "gsc_github_dorks.py"), args.org,
                   "--limit", str(args.limit), "--days", str(args.days)]
            subprocess.run(cmd)

    elif args.command == "agentless":
        cmd = [sys.executable, str(_GSC_ROOT / "gsc_agentless.py"), args.host]
        ...
        cmd = [sys.executable, str(_GSC_ROOT / "gsc_threat_model.py"), args.repo]
        ...
        cmd = [sys.executable, str(_GSC_ROOT / "gsc_deep_reducer.py"), args.target]
        if hasattr(args, 'model'): cmd += ["--model", args.model]
        ...
        ext_args = [sys.executable, str(_GSC_ROOT / "gsc_external.py"),
        ...
        subprocess.run([sys.executable, str(_GSC_ROOT / "scripts" / "gsc_calibration.py"),
                        "run",
