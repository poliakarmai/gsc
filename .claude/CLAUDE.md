...
## Codebase Intelligence for gsc (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-06 (commit f50716d)
...
- `gsc.py` — 8 bug fixes, last fix today (bug magnet); 42 commits/90d
- `gsc_external.py` — 4 bug fixes, last fix today (bug magnet); 17 commits/90d
- `scripts/gsc_calibration.py` — 3 bug fixes, last fix yesterday; 5 commits/90d
- `gsc_detectors/registry.py` — 3 bug fixes, last fix today; 15 commits/90d
- `gsc_detectors/gs020_llm_sqli.py` — 2 bug fixes, last fix yesterday; 4 commits/90d

### Code health
Three co-equal signals: defect risk 8.76/10 avg, hotspot health 5.18/10 (stable), worst `gsc_external.py` at 2.66/10 · maintainability 8.95/10 · performance risk 131 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `gsc_detectors/gs020_llm_sqli.py` — nested complexity (_get_api_key) — impact −2.5
- `gsc.py` — prior defect — impact −2.0
- `gsc_external.py` — prior defect — impact −2.0
- `gsc_detectors/registry.py` — prior defect — impact −2.0
- `scripts/gsc_calibration.py` — prior defect — impact −2.0
