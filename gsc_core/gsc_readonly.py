"""Conservative read-only shell-command classifier for GSC sandbox grants.

Ported from openworker (MIT, ``coworker/readonly.py``). Used by the Proof-of-Fix /
self-healing sandbox to auto-allow *only* local read-only commands without a hand
approval, while blocking anything that can write, execute, or mutate — and every network
client, because an auto-allowed network command is an exfiltration channel under prompt
injection.

Contract (fail-closed):
  * Local filesystem reads only. Network clients (curl/wget/ssh/nc) excluded even for
    GET. Interpreters (python/sh -c) and anything write/exec/mutate are excluded.
  * Pipelines allowed (``nl | sed -n | grep``) — every stage must classify. All other
    shell operators (;, &&, ||, &, redirections, substitutions) reject outright.
  * Fail closed: unknown commands, unparseable input, path-invoked binaries, doubtful
    flags reject. A false negative costs one manual approval; a false positive costs an
    unreviewed side effect — that asymmetry decides every edge case.

This is a user-elected convenience on top of the sandbox, not a sandbox by itself: the
command still runs under the sandbox's no-network/timeout policy.
"""
...
# Git subcommands that only read. Per-subcommand guards below — several git "read"
# commands grow write/exec behavior through specific flags.
_GIT_SAFE = {
...
# command may READ, so a grant meant for "stop asking about my project files" also
# covered ~/.aws/credentials. These helpers name the file operands so the caller can hold
# them to the sandbox's allowed roots.
...
def read_targets(command: str) -> list:
    """Every file operand `command` would read, for scoping against the sandbox roots.
