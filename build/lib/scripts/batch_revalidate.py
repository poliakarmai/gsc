...
def fetch_findings(limit=BATCH_SIZE):
    """Output findings as JSON for agent to classify."""
    db = sqlite3.connect(DB)
    db.row_factory = sqlite3.Row
    
    rows = db.execute("""
        SELECT f.id, f.category, f.title, f.project, f.file_path, 
               f.line_number, f.detail, f.echelon
        FROM findings f
...
def update_db(results_json):
    ...
    for r in rules:
        ...
        if tp_rate < 0.30:
            ...
            existing = db.execute(
                "SELECT 1 FROM federated_deactivated WHERE rule_id = ?", (r['category'],)
            ).fetchone()
            if not existing:
                db.execute(
                    "INSERT INTO federated_deactivated (rule_id, reason, deactivated_at) VALUES (?, ?, ?)",
                    (r['category'], reason, now)
                )
                stats[f"DEACTIVATED_{r['category']}"] = 1
                print(f"  🛑 AUTO-DEACTIVATED: {r['category']} (TP={tp_rate:.2f} at {r['total']} verdicts)")
