...
## Замер 5 (26.08.2026) — перезамер из БД (0 LLM, 0 scan)

Пересчёт precision из существующих findings БД (`scripts/gsc_precision_rescan.py`),
без повторного скана. Данные — полный rescan 99 проектов уже с пофикшенными детекторами.

| Метрика | Замер 3 (21.08) | Замер 5 (26.08) |
|---|---|---|
| Clean проектов | 90 | 90 |
| Clean с ложным CRITICAL | 48/90 (53%) | **42/90 (47%)** |
| Recall | 8/10 | **10/10** |
| Precision CRIT (по находкам) | ~4–5% | **20.2%** (TP=362, FP=1427) |

**Вывод:** precision-чистка дала ~4× рост precision CRIT (4–5% → 20%) и идеальный recall
(10/10). Остаточный шум: 42/90 чистых всё ещё дают ≥1 ложный CRITICAL. Новый лидер —
**GS001 (hardcoded secret, 849 CRIT)** → следующий кандидат на тюнинг, за ним GS005 (312).
