...
## Замер 6 (26.08.2026) — полный rescan 115 + GS001-тюнинг

Полный re-scan всех 115 targets (calibration + bench_100) под новыми детекторами
(`gsc_rescan_benchmark.py --reset`, 0 LLM, ~2ч), затем пересчёт `gsc_precision_rescan.py`.
GS001-тюнинг: exclude gettext `.po` + фильтр AWS demo/lowercase-ключей.

| Метрика | Замер 3 (21.08) | Замер 5 (до rescan) | **Замер 6** |
|---|---|---|---|
| Clean с ложным CRITICAL | 48/90 (53%) | 42/90 (47%) | **42/90 (47%)** |
| Recall | 8/10 | 10/10 | **10/10** |
| Precision CRIT (по находкам) | ~4–5% | 20.2% | **29.1%** (TP=172, FP=419) |

**Вывод:** GS001-тюнинг сработал — FP упал 849 → **87** CRIT (~10×). Precision CRIT
вырос 4–5% → **29.1%** (~6× от исходного), recall идеальный 10/10. Новый лидер шума —
**GS005 (150)**, за ним GS001 (87), GS029 (47). Clean с ≥1 ложным CRITICAL держится 47% —
дальше точечно: GS005 интерполяция без taint, GS029 secrets.
