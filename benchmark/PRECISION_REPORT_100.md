...
## Замер 7 (26.08.2026) — GS001 + GS005/GS029 downgrade (финал)

Полный re-scan 115 + GS001-тюнинг (gettext `.po`, AWS demo) + downgrade двух
дубликатов: GS005 `f-string in query` и GS029 `Hardcoded encryption key`
(CRITICAL→HIGH; синхронизирован рассинхрон код↔БД-сид).

| Метрика | Замер 3 (21.08) | Замер 6 | **Замер 7** |
|---|---|---|---|
| Clean с ложным CRITICAL | 48/90 (53%) | 42/90 (47%) | **38/90 (42%)** |
| Recall | 8/10 | 10/10 | **10/10** |
| Precision CRIT (по находкам) | ~4–5% | 29.1% | **31.1%** (TP=135, FP=299) |

**Вывод:** precision CRIT вырос 4–5% → **31.1%** (~7×), recall идеальный 10/10.
Остаточный шум: GS001 (87), GS005 (77), GS020 (26), GS037 (23) — убывающая отдача,
дальше точечно по мере необходимости.
